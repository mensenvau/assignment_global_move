...
exports.getPlayers = async (req, res, next) => {
  try {
    ...
    let [cnt, players] = await Promise.all([execute("select count(*) as count from players order by player_id asc", [], 1), execute("select * from players order by player_id asc limit ?, ?", [size * page, size])]);
    if (!players) throw new Error("This players does not exist");
